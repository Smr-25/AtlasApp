...
) : IRequestHandler<AddMigrationCommand, string>
{
    ...
    {
        var project =
            await applicationDbContext.ProjectProfiles.FirstOrDefaultAsync(x => x.Id == request.ProjectId,
                cancellationToken);
        ...
        var result = await scriptRunner.ExecuteAsync("dotnet", command.Replace("dotnet ", ""), project.RootPath,
            cancellationToken);
        ...
        return $"✅ Migration '{migrationName}' added successfully.\n\nCommand: {command}\n\nResult:\n{result}";
    }
}