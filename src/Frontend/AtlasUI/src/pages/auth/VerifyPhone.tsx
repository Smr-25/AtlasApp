...
export default function VerifyPhone() {
  ...
  const resend = async (channel?: 'Sms' | 'Telegram') => {
    if (!phone) {
    ...
    if (!selected) {
      // ask user via prompt (simple) — for better UX replace with modal
      const choice = window.prompt('Resend via (sms/telegram)? Type sms or telegram');
      if (!choice) return;
      selected = choice.toLowerCase().startsWith('t') ? 'Telegram' : 'Sms';
    }
