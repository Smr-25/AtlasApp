...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const errs = await login(identifier, password);
    if (errs.length === 0) {
      // ProtectedRoute / RoleRouter will handle the redirect logic
      navigate("/dashboard");
    } else if (errs.includes("EMAIL_NOT_VERIFIED")) {
  ...
  return (
    <AuthLayout title="Welcome back" subtitle="Sign in to your Atlas workspace">
